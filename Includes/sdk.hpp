...
#include <d3d9.h>
#include <vector>
#include "niggachees.h"
#include <iostream>
#include "settings.hpp"
#include "../driver.h"

// u gotta define PI here cause i forgot
...
/*D*//*3DMATRIX Matrix(Vector3 rot, Vector3 origin = Vector3(0, 0, 0))
{
...
bool IsVisible(uintptr_t mesh) //basiclly we are gonna change from read to read okay so lets see if we will have any errors
{
	float last_sumbit_time = read<float>(mesh + 0x360);
	float last_render_time_on_screen = read<float>(mesh + 0x368);
...
{
	char v1; // r8
	CamewaDescwipsion ViewPoint = read<CamewaDescwipsion>(baseaddy + 0xE9AD420);
...
std::string find_object_class(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(base_address + 0x10376500 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
	...
	if ((uint32_t)nameLength)
	{
		...
		v5 = read<int>(base_address + 0x102CE0A8);
		v6 = (v5 << 8) | (v5 >> 8);
		v7 = v5 >> 15;
		if (v4) {
			v8 = v4;
...
std::string find_object_name(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(base_address + 0x10376500 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); // ERROR_NAME_SIZE_EXCEEDED
