...
void game_loop()
{
	...
	{
		char fps[256]; // 64
		sprintf_s(fps, "Pasta School", ImGui::GetIO().Framerate); 
	...
	pointer::uworld = read<uintptr_t>(baseaddy + offsets::Uworld); //here we have error cause we didnt include the driver.h
	//if (!pointer::uworld) {
	...
	pointer::local_players = read<uintptr_t>(read<uintptr_t>(pointer::game_instance + offsets::local_players));

	pointer::player_controller = read<uintptr_t>(pointer::local_players + offsets::PlayerController);

	pointer::local_pawn = read<uintptr_t>(pointer::player_controller + offsets::AcknowledgedPawn);

	pointer::root_component = read<uintptr_t>(pointer::local_pawn + offsets::RootComponent);

	pointer::player_state = read<uintptr_t>(pointer::local_pawn + offsets::PlayerState);
	...
	pointer::AFortPlayerState = read<uint64_t>(pointer::local_pawn + offsets::PlayerState);

	pointer::local_relative_location = read<Vector3>(pointer::root_component + offsets::RelativeLocation);

	pointer::ULevel = read<uint64_t>(baseaddy + Offsets::PersistentLevel);

	pointer::Actors = read<uintptr_t>(pointer::ULevel + offsets::AActor);

	pointer::closest_distance = FLT_MAX;

	pointer::closest_pawn = NULL;

	int count = read<int>(pointer::game_state + (0x02a0 + sizeof(uintptr_t)));
	for (int i = 0; i < count; i++)
	{
		auto current_actor = read<uint64_t>(pointer::Actors + i * 0x8);

		int PlayerState = read<uint64_t>(current_actor + offsets::PlayerState);

		uintptr_t current_player_state = read<uintptr_t>(pointer::player_array + (i * sizeof(uintptr_t)));

		if (!current_player_state) continue;

		int my_team_id = read<int>(pointer::player_state + offsets::TeamID); //TeamIndex

		int current_actor_team_id = read<int>(current_player_state + offsets::TeamID); //TeamIndex
		...
		uintptr_t current_mesh = read<uintptr_t>(current_local_pawn_private + offsets::mesh);
		...
		int screenx = 0;
		int screeny = 0; //Okay so we swapped our reading method from previous driver and now all we need to do else is getting pid (processid) and base 
		...
		uint64_t CurActorRootComponent = read<uint64_t>(Actor + offsets::RootComponent);
		localactorpos = read<Vector3>(pointer::root_component + offsets::RelativeLocation);
		...
		uint64_t Mesh2 = read<uint64_t>(current_actor + mesh);
		...
		if (settings::visuals::enable)
		{
			...
			if (settings::visuals::box)
			{
				if (settings::visuals::vischeck)
				{
					...
					if (IsVisible(current_mesh))
					{
						DrawNormalBox(Headbox1.x - BoxWidth / 2, Headbox1.y - 1, BoxWidth, BoxHeight, 1.0f, ImColor({ settings::visuals::VisCOlor[0], settings::visuals::VisCOlor[1], settings::visuals::VisCOlor[2], settings::visuals::VisCOlor[3] }), settings::visuals::boxwidth);
					
					}
					else
					{
						DrawNormalBox(Headbox1.x - BoxWidth / 2, Headbox1.y - 1, BoxWidth, BoxHeight, 1.0f, ImColor({ settings::visuals::NOtVIs[0], settings::visuals::NOtVIs[1], settings::visuals::NOtVIs[2], settings::visuals::NOtVIs[3] }), settings::visuals::boxwidth);
			...
			if (settings::visuals::cornered_box)
			{
				if (settings::visuals::vischeck)
				{
					if (IsVisible(current_mesh))
					{
						...
						draw_cornered_box(Headbox1.x - (BoxWidth / 2), Headbox1.y, BoxWidth, BoxHeight, ImColor({ settings::visuals::VisCOlor[0], settings::visuals::VisCOlor[1], settings::visuals::VisCOlor[2], settings::visuals::VisCOlor[3] }), settings::visuals::boxwidth, 0);
					}
					else
					{
						float BoxHeight = Rootbox.y - Headbox1.y;
						...
						draw_cornered_box(Headbox1.x - (BoxWidth / 2), Headbox1.y, BoxWidth, BoxHeight, ImColor({ settings::visuals::NOtVIs[0], settings::visuals::NOtVIs[1], settings::visuals::NOtVIs[2], settings::visuals::NOtVIs[3] }), settings::visuals::boxwidth, 0);
					}
				}
				else
		...
		{
			int keyid = VK_XBUTTON2;
			switch (settings::aimbot::key_wow) {
			case 0:
				keyid = VK_LBUTTON; 
				break;
			case 1:
